...
-- Создание внешнего сервера, который подключается к 'oltp'
CREATE SERVER oltp_server
    FOREIGN DATA WRAPPER postgres_fdw
    OPTIONS (host 'localhost', dbname '01', port '5432');

-- Создание пользовательского отображения для текущего пользователя
CREATE USER MAPPING FOR CURRENT_USER
    SERVER oltp_server
    OPTIONS (user 'postgres', password '159357');

-- Импорт схемы из внешнего сервера
